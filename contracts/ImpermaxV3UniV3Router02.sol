...
import "./interfaces/IUniV3SwapRouter02.sol";
...
contract ImpermaxV3UniV3Router02 is ImpermaxV3BaseRouter02 {
	address public uniswapV3Factory;
	address public swapRouter;
	
	// transaction data
	uint24 private storedFee;
	
	constructor(address _factory, address _uniswapV3Factory, address _WETH, address _vaultFactory, address _swapRouter) public ImpermaxV3BaseRouter02(_factory, _WETH, _vaultFactory) {
	...
	function _redeemUniV3Step1(
		...
		if (percentage == 1e18) {
			(uint24 fee,,,,,,,) = ITokenizedUniswapV3Position(pool.nftlp).positions(tokenId);
			storedFee = fee;
		}
	...
	function _swap(
		...
		uint24 fee = storedFee;
		if (fee == 0) (fee,,,,,,,) = ITokenizedUniswapV3Position(pool.nftlp).positions(tokenId);
		
		address tokenIn = pool.tokens[indexOut == 0 ? 1 : 0];
		_updateAllowance(tokenIn);
		
		IUniV3SwapRouter02(swapRouter).exactOutputSingle(IUniV3SwapRouter02.ExactOutputSingleParams({
			tokenIn: tokenIn,
