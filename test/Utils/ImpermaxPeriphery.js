...
async function mintCollateralETHAero(router, nftlp, borrower, tokenId, tickSpacing, tickLower, tickUpper, amountADesired, amountBDesired, amountAMin, amountBMin, A_IS_0, ETH_IS_0, permits) {
	const t = getAmounts({amountADesired, amountBDesired, amountAMin, amountBMin}, A_IS_0);
	let amountETH = ETH_IS_0 ? t.amount0Desired : t.amount1Desired;
	const actions = [
		await routerManager.actionsGetter.getMintUniV3Action(t.amount0Desired, t.amount1Desired, t.amount0Min, t.amount1Min),
...
async function mintAndLeverageETHAero(router, nftlp, borrower, tokenId, tickSpacing, tickLower, tickUpper, amountAUser, amountBUser, amountADesired, amountBDesired, amountAMin, amountBMin, A_IS_0, ETH_IS_0, permits) {
	const t = getAmounts({amountAUser, amountBUser, amountADesired, amountBDesired, amountAMin, amountBMin}, A_IS_0);
	let amountETH = ETH_IS_0 ? t.amount0Desired : t.amount1Desired;
	const actions = [
		await routerManager.actionsGetter.getBorrowAndMintUniV3Action(t.amount0User, t.amount1User, t.amount0Desired, t.amount1Desired, t.amount0Min, t.amount1Min),
	...
	return execute(router, nftlp, borrower, tokenId, actions, permits, true, amountETH);
}

async function redeemCollateralETHAero(router, nftlp, borrower, tokenId, percentage, amountAMin, amountBMin, A_IS_0, ETH_IS_0, permits) {
	const t = getAmounts({amountAMin, amountBMin}, A_IS_0);
	const lendingPool = await router.getLendingPool(nftlp.address);
	const tokenToWithdraw = ETH_IS_0 ? lendingPool.tokens[1] : lendingPool.tokens[0];
	
	const actions = [
		await routerManager.actionsGetter.getRedeemUniV3Action(percentage, t.amount0Min, t.amount1Min, router.address),
		await routerManager.actionsGetter.getWithdrawEthAction(borrower),
		await routerManager.actionsGetter.getWithdrawTokenAction(tokenToWithdraw, borrower)
	];
...
async function deleverageETHAero(router, nftlp, borrower, tokenId, percentage, amountAMin, amountBMin, A_IS_0, ETH_IS_0, permits) {
	const t = getAmounts({amountAMin, amountBMin}, A_IS_0);
	const lendingPool = await router.getLendingPool(nftlp.address);
	const tokenToWithdraw = ETH_IS_0 ? lendingPool.tokens[1] : lendingPool.tokens[0];
	...
	const actions = [
		await routerManager.actionsGetter.getRedeemUniV3Action(percentage, t.amount0Min, t.amount1Min, router.address),
		await routerManager.actionsGetter.getRepayRouterAction(0, MAX_UINT_256, ETH_IS_0 ? router.address : borrower),
		await routerManager.actionsGetter.getRepayRouterAction(1, MAX_UINT_256, !ETH_IS_0 ? router.address : borrower),
		await routerManager.actionsGetter.getWithdrawEthAction(borrower),
		await routerManager.actionsGetter.getWithdrawTokenAction(tokenToWithdraw, borrower)
	];
